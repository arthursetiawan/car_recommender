...
def scrape_cars_com():
    ...
    if response.status_code == 200:
        ...
        cars = []
        for listing in soup.find_all("div", class_="vehicle-card"):  # Update class name as needed
            title_tag = listing.find("h2", class_="title")
            title = title_tag.text.strip() if title_tag else "Unknown"
            
            price_tag = listing.find("span", class_="primary-price")
            price = price_tag.text.strip() if price_tag else "N/A"
            
            mileage_tag = listing.find("div", class_="mileage")  # Mileage info
            ...
            location_tag = listing.find("div", class_="dealer-name")
